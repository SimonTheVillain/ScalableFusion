...
void TumDataset::readNewSetOfImages()
{
    ...
    if(frameIndex < rgbFiles.size()){
        ...
        if(!depthUndistort1.empty()){
            ...
            cv::remap(currentDepth, currentDepthUndistorted, depthUndistort1, depthUndistort2, cv::INTER_NEAREST);
        ...
        if(!rgbUndistort1.empty()){
            ...
            cv::remap(currentRGB,currentRgbUndistorted,rgbUndistort1,rgbUndistort2,cv::INTER_LINEAR);
