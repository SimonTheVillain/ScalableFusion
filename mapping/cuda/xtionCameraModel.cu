...
__global__ void generateXtionConfidenceImage_kernel(const cudaSurfaceObject_t rawDepth,cudaSurfaceObject_t output,
    ...
    /*
    float4 debug=make_float4(1,1,1,1);
    surf2Dwrite(debug,output,x*sizeof(float4),y);
    return;
    */
    ...
    float depth;
    surf2Dread(&depth,rawDepth,x*sizeof(float),y);
...
void generateXtionConfidenceImage(const cudaSurfaceObject_t rawDepth,cudaSurfaceObject_t output,int width,int height){
    gpuErrchk( cudaPeekAtLastError() );
    dim3 block(32,32);
    dim3 grid((width+block.x-1)/block.x,(height+block.y-1)/block.y);

    generateXtionConfidenceImage_kernel<<<grid,block>>>(rawDepth,output,width,height);
...
__global__ void generateXtionConfidenceImage16F_kernel(const cudaSurfaceObject_t rawDepth,cudaSurfaceObject_t output,
                                                    int width,int height){
    ...
    ushort4 result = make_ushort4(
                __float2half_rn(color.x),
...
void generateXtionConfidenceImage16F(const cudaSurfaceObject_t rawDepth,cudaSurfaceObject_t output,int width,int height){
    assert(0);
    dim3 block(32,32);
    dim3 grid((width+block.x-1)/block.x,(height+block.y-1)/block.y);

    generateXtionConfidenceImage16F_kernel<<<grid,block>>>(rawDepth,output,width,height);
    cudaDeviceSynchronize(); //maybe we should work with streams, but what do i know?
